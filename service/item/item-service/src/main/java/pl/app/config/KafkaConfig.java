    ...
    class TopicConfiguration {
        ...
        KafkaAdmin.NewTopics createTopics(KafkaTopicConfigurationProperties topicNames) {
            NewTopic[] array = Stream.of(
                    ...
                    createTopicFromConfig(topicNames.getOutfitTemplateUpdated()).stream(),
                    createTopicFromConfig(topicNames.getOutfitTemplateDeleted()).stream(),
                    createTopicFromConfig(topicNames.getOutfitCreated()).stream(),
                    createTopicFromConfig(topicNames.getOutfitDeleted()).stream(),

                    createTopicFromConfig(topicNames.getWeaponTemplateCreated()).stream(),
                    createTopicFromConfig(topicNames.getWeaponTemplateUpdated()).stream(),
                    createTopicFromConfig(topicNames.getWeaponTemplateDeleted()).stream(),
                    createTopicFromConfig(topicNames.getWeaponCreated()).stream(),
                    createTopicFromConfig(topicNames.getWeaponDeleted()).stream(),

                    createTopicFromConfig(topicNames.getEquipmentCreated()).stream(),
                    createTopicFromConfig(topicNames.getCharacterGearAddedToEquipment()).stream(),
                    createTopicFromConfig(topicNames.getCharacterGearRemovedFromEquipment()).stream(),
                    createTopicFromConfig(topicNames.getEquipmentItemAdded()).stream(),
                    createTopicFromConfig(topicNames.getEquipmentItemRemoved()).stream(),

                    createTopicFromConfig(topicNames.getTraderCreated()).stream(),
                    createTopicFromConfig(topicNames.getTraderItemsRenewed()).stream(),
                    createTopicFromConfig(topicNames.getGodBoughtItem()).stream(),
                    createTopicFromConfig(topicNames.getGodSoldItem()).stream(),

                    createTopicFromConfig(topicNames.getGearCreated()).stream(),
                    createTopicFromConfig(topicNames.getGearRemoved()).stream(),
                    createTopicFromConfig(topicNames.getGearItemSet()).stream(),
                    createTopicFromConfig(topicNames.getGearItemRemoved()).stream(),
