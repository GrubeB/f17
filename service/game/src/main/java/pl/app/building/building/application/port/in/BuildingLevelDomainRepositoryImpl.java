...
import static pl.app.building.building.application.domain.BuildingType.HEADQUARTERS;
...
public class BuildingLevelDomainRepositoryImpl implements BuildingLevelDomainRepository {
    ...
    private final Set<ResourceLevel> clayPitLevels = Set.of(
            new ResourceLevel(1, new Resource(30, 50, 40, 1), Duration.of(30, SECONDS), new HashSet<>(), 120),
    ...
    public Mono<Map<Integer, ? extends BuildingLevel>> fetchAll(BuildingType type) {
        var levels = switch (type) {
            ...
            case CLAY_PIT -> clayPitLevels;
            case IRON_MINE -> ironMineLevels;
            case FARM -> farmLevels;
            case WAREHOUSE -> warehouseLevels;
            case CHAPEL -> chapelLevels;
            case CHURCH -> churchLevels;
            case RALLY_POINT -> rallyPointLevels;
            case BARRACKS -> barrackLevels;
            case STATUE -> statueLevels;
            case WALL -> wallLevels;
            case HOSPITAL -> hospitalLevels;
            case MARKET -> marketLevels;
