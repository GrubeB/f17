...
class ArmyWalkServiceImplTest {//extends AbstractIntegrationTest {
    ...
    void sendArmy() {
        ...
        StepVerifier.create(service.send(new ArmyWalkCommand.SendArmyCommand(
                        ArmyWalkType.ATTACK,
                        village1.getId(),
                        village2.getId(),
    ...
    void sendArmy_shouldThrow_whenThereIsNotEnoughOfficers() {var player = playerService.create(PlayerCreateDto.builder().accountId(ObjectId.get().toHexString()).build()).block();
        ...
        StepVerifier.create(service.send(new ArmyWalkCommand.SendArmyCommand(ArmyWalkType.ATTACK, village1.getId(), village2.getId(),
    ...
    void sendArmy_shouldSendArmyAndRemoveItem_whenThereOfficers() {var player = playerService.create(PlayerCreateDto.builder().accountId(ObjectId.get().toHexString()).build()).block();
        ...
        var village2 = villageService.cratePlayerVillage(new VillageCommand.CreatePlayerVillageCommand(player.getPlayerId())).block();
        villageArmyService.add(new VillageArmyCommand.AddUnitsCommand(village1.getId(), Army.of(Map.of(UnitType.SPEARMAN, 100)))).block();
        inventoryService.add(new InventoryCommand.AddItemCommand(player.getPlayerId(), new OfficerItem(OfficerType.GRANDMASTER), 1)).block();
    ...
    void processAttack() {
        ...
        ArmyWalk armyWalk = service.send(new ArmyWalkCommand.SendArmyCommand(
    ...
    void sendArmy_shouldConquerVillage() {
        ...
        ArmyWalk armyWalk1 = service.send(new ArmyWalkCommand.SendArmyCommand(ArmyWalkType.ATTACK, village1.getId(), village2.getId(), Army.of(Map.of(UnitType.SPEARMAN, 10, UnitType.NOBLEMAN, 1)), Resource.zero(), new Officers())).block();
        ArmyWalk armyWalk2 = service.send(new ArmyWalkCommand.SendArmyCommand(ArmyWalkType.ATTACK, village1.getId(), village2.getId(), Army.of(Map.of(UnitType.SPEARMAN, 10, UnitType.NOBLEMAN, 1)), Resource.zero(), new Officers())).block();
        ArmyWalk armyWalk3 = service.send(new ArmyWalkCommand.SendArmyCommand(ArmyWalkType.ATTACK, village1.getId(), village2.getId(), Army.of(Map.of(UnitType.SPEARMAN, 10, UnitType.NOBLEMAN, 1)), Resource.zero(), new Officers())).block();
        ArmyWalk armyWalk4 = service.send(new ArmyWalkCommand.SendArmyCommand(ArmyWalkType.ATTACK, village1.getId(), village2.getId(), Army.of(Map.of(UnitType.SPEARMAN, 10, UnitType.NOBLEMAN, 1)), Resource.zero(), new Officers())).block();
