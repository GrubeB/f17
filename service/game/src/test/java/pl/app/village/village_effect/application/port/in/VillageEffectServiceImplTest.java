...
class VillageEffectServiceImplTest extends AbstractIntegrationTest {
    ...
    void start() {
        ...
        StepVerifier.create(service.start(new VillageEffectCommand.StartEffectCommand(village.getId(), EffectType.WOOD_BUFF, Duration.ofDays(2), 20)))
                .assertNext(next -> {
    ...
    void remove() throws InterruptedException {
        Village village = villageService.cratePlayerVillage(new VillageCommand.CreatePlayerVillageCommand(ObjectId.get())).block();
        service.start(new VillageEffectCommand.StartEffectCommand(village.getId(), EffectType.WOOD_BUFF, Duration.ofSeconds(2), 20)).block();
        Thread.sleep(4000);
